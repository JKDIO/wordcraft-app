...
> **현재 상태 스탬프 → 라이브 v1.4.18 (2026-08-13, ★특별 구역 포탈 — 단어 대륙·소리 훈련소를 알림 줄에서 들어가는 문으로 승격★).**
> v1.4.18 = Dio님 지적("알림 메시지 띄워놓은 것 마냥 되어 있다")에 따른 UI 위계 수정. 두 구역을 2열 포탈 카드로(큰 아이콘·진행도·발광 펄스·NEW 리본·CTA 알약). app.css 무변경(keyframes는 `<style>` 주입).
> ⚠️ **배포 절차 추가**: 빌드 후 번들에 박힌 APP_VERSION과 version.json을 **grep으로 대조**할 것. v1.4.18 빌드 직전 version.ts가 1.4.16으로 되돌아가 있는 것을 발견했다(그대로 나갔으면 예한이 앱에 "업데이트 있음" 배너가 영구히 떴다). 컨테이너 파일이 되돌아가는 현상이 실제로 있었다 — **빌드 직전 버전 확인은 선택이 아니다.**
> **(v1.4.17) 관제실 어휘 XP 집계 봉합 + CONTRACT v1.5.**
> v1.4.17 = v1.4.16에서 빠뜨린 L12 위반을 자체 적발·수정. 관제실이 어휘 XP를 문항당 10(실제 5)·팩 50(실제 30)으로 잡아 **오늘 XP를 +310으로 표시(실제 205)** 하던 결함. 학습 화면·콘텐츠·음원은 불변. **XP 산식 1:1 자동 대조 테스트를 신설**했으니 앞으로 XP를 건드리면 그걸로 판정할 것.
> **(v1.4.16, 2026-08-12) 단어 대륙 오픈 + 월드맵 접기 + 가족 세션 봉합.**
> ① **단어 대륙 🗺️(`#/vocab`) 오픈 완료** — GIU 전체 + 중학 전 과정 **2,400단어(10티어 × 20팩 × 12단어)**. 교차 검수 200/200팩 완료, **음원 4,799개 파형 전수 감사 PASS**, `version.json`의 `vocab_ready:true`로 예한이 월드맵에 배너 노출 확인.
...
> ⚠️ **최종 판정은 아직 안 났다 — 예한이 폰에서의 실사용 확인이 남았다.** 자동화 브라우저는 자동재생이 차단돼 실제 재생을 계측으로 증명하지 못했다(파형 감사는 파일에 소리가 있음을 증명한 것이지, 예한이 폰에서 들린다는 증명이 아니다).
> (이전 스탬프) v1.4.15 (2026-08-01, 무음 음원 전면 봉합 + 폰 TTS 의존 완전 제거).
> ⚠️ 소스 SSOT = 노트북 `Word Craft/wordcraft-app/_dev_github/src_v1.4.16/`(+ `wordcraft_src_v1.4.16.tgz`). **콘텐츠 SSOT = `wordcraft_content_v1.4.15.json`(불변) · 어휘 SSOT = `wordcraft_vocab_v1.4.16_final.json` + `vocab_packs_v3_final.tgz`(검수 완료 200팩 + SPEC + 진행기록).**

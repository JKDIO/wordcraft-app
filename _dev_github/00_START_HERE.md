...
> **현재 상태 스탬프 → 라이브 v1.4.20 (2026-08-13, ★단어 대륙에 이야기가 생겼다 + 보스전★).**
> v1.4.20 = Dio님 지시("스토리·새 모드·보스전을 완전무결하게, 단 아빠 대결은 제외")에 따른 대형 확장.
>  ① **이야기 레이어** — 세계관(무자가 말을 삼킨다 / 예한이는 말잡이) + **티어 수호자 10명**(팩 5개마다 각성 5단계) + **팩 결말 200편 + 수호자 한마디 200편**. 팩 **도입 화면 신설**(그동안 `intro_ko`는 데이터에만 있고 화면에 없었다).
>  ② **분류 상자 🗃️** — 네 번째 입력 방식. 12칩을 한 화면에서 품사(또는 음절)로 나눠 담는 귀납 학습. **189/200팩** 적용.
>  ③ **속사 사냥 🔥** — 45초 선택형 유창성 라운드. **의도적으로 기록·XP 없음**(지표 오염·찍기 습관 방지).
>  ④ **단어 골렘 ⚔️** — 팩 5개마다(전체 40회). 직전 5팩 60단어 중 12개. 틀리면 큐 뒤로 → **실패 없는 완전 학습**. 길은 막지 않는다.
>  ⑤ **해금 곡선을 누적 팩 수 기준으로** — 첫 7팩 동안 거의 매 팩 새로운 게 하나씩 열린다(전에는 T2까지 16팩 동안 3가지뿐).
>  ⑥ **★P0 회귀 봉합★** — v1.4.17 관제실 어휘 XP 수정이 v1.4.18에서 유실돼 라이브에서 XP가 부풀어 있었다. XP 산식을 `lib/xp.ts` 하나로 통합 + **자동 패리티 검사** 신설(L27).
> 📊 전수 계측: 탭 비율 **50% → 33.5%**(티어별 31.2~35.2%) · 정답 위치 25.67/24.89/25.91/23.54% · 골렘 40세션·속사 200팩·문항 무결성 **결함 0** · XP 패리티 통과 · tgz→클린빌드 md5 동일.
> ⚠️ **아직 안 검증한 것**: **예한이가 실제로 재미있어하는지** — 구조와 분량은 만들었지만 재미의 증명은 아니다. 실반응 데이터 0.
> ⚠️ **배포 절차(필수 순서)**: 타입검사 → **`node xp_parity.mjs`** → 빌드 → **번들 버전 grep 3자 대조(L25)** → 렌더 스모크 → 배포.
> ⚠️ 소스 SSOT = 노트북 `_dev_github/src_v1.4.20/`(+ tgz). 어휘 SSOT = `wordcraft_vocab_v1.4.20.json`(스토리 포함) + `vocab_packs_v3_final.tgz`. 세계관은 `claude/단어대륙_월드바이블_v1.md`.

## ⚡ 지금 최우선 (2026-08-13~)
1. **예한이 폰 실사용 확인** — ① 첫 팩에서 **소리가 실제로 나는지** ② 새로 생긴 **도입/결말 화면을 읽는지, 건너뛰는지** ③ **분류 상자**를 손가락으로 다룰 만한지 ④ 15~20분에 몇 팩을 하는지. 이게 최종 판정이다.
2. **골렘 첫 등장 확인** — 예한이가 팩 5개를 끝내면 T1에 흙덩이 골렘이 뜬다. 실제로 들어가는지, 12칸을 다 깨는지(중간에 나가지 않는지) 관찰.
3. **관제실 XP 확인** — v1.4.20부터 숫자가 정확하다. 그 전(8/12~8/13)에 보신 XP는 부풀려진 값이었다. 소급 보정은 하지 않았다.
4. **청취 확인표(HTML) 검토** — 재생성 4개(tuesday·sweat·let·leader)는 원래 무음이었던 것이라 **반드시** 확인.
5. **가족 기능** — L23 봉합은 라이브 실증 완료. 남은 것은 ① 예한이 폰(WebView) 동일 확인 ② **진영이네 부모 실제 로그인 왕복**(아직 guardians=0).
6. **데이터 정리(Dio님 승인 필요)** — 준비 완료, **미실행**. `앱개발/데이터정리_승인대기_20260813.md` 참조.
7. **이월 과제** — `public/app.css`↔배포본 크기 불일치 · v1.4.2 소환진 픽셀 델타 · Phase 4(PWA 서비스워커) · v1.4.15 재생성 클립 11개 청취 확인 · 4지선다 정답 위치 4번째 칸 23.54%(하한 근접, 다음 릴리스에서 재확인) · 분류 상자 음절 기준 2,400단어 전수 대조.
8. **나중에 검토(지금 안 함)** — 아빠와의 단어 대결(Dio님 제외 지시) · 적응형 문항 수 · 수호자 음성.
...
**낡지 않는 진실:**
1. **프로젝트 메모리 `RELEASE_LOG.md`** (append-only, 맨 위=최신 = **v1.4.20**).
2. 라이브 `https://wordcraft-app.vercel.app/version.json` (WebFetch 시 캐시버스터 `?cb=...` 필수). 현재 **1.4.20**.
3. 소스 `APP_VERSION` = 로컬 `src_v1.4.20/src/lib/version.ts` = '1.4.20'.
...
- **라이브 v1.4.20**: 학습(유령 보스·수정 동굴·소리 훈련소·문장 소환진·복습 라이트너·출석 15분) + **단어 대륙(2,400단어/200팩) + 워드몬 진화 + 단어 도감 + 이야기(수호자 10·팩 결말 200) + 분류 상자 + 속사 사냥 + 단어 골렘 40** + 다가구 인증 라우팅 + 가족 RLS 격리 + **전 재생 항목 음원화**.
...
## 2. 🔒 철칙 (어기면 사고 재발)
1. **소스 SSOT = 로컬 `_dev_github/src_v1.4.20/`(+ tgz), 콘텐츠 SSOT = `wordcraft_content_v1.4.15.json` + 어휘 `wordcraft_vocab_v1.4.16_final.json`.** 컨테이너에서만 고쳐 배포 금지.
2. **두 앱 공동 기록** (RELEASE_LOG에 예한이 앱+관제실+연동 함께).
3. **연동 계약 준수**(`앱개발/연동계약_CONTRACT_v1.md`, 현재 **v1.5** — §13에 어휘 XP·기록 규약). v1.4.20은 **XP에 `vocab_golem`(+40)이 추가**됐다 — CONTRACT §13 갱신 필요(다음 세션 과제). 스키마·기록 방식은 불변.
...
| 위치 | 내용 |
|---|---|
| 로컬 `Word Craft/wordcraft-app/` | GitHub 배포 저장소 클론. 루트 main.js·content.json·**vocab.json(1.44MB)**·version.json = **v1.4.20** |
| `_dev_github/src_v1.4.20/` | **소스 SSOT** — App.tsx·screens/{…,VocabContinent,WordDex}·engine/·lib/{audio,tts,store,supabase,version,**vocab,wordmon**}·**tools/음원감사_AUDIO_QA.html** |
| `_dev_github/wordcraft_src_v1.4.20.tgz` | 소스 스냅샷. tgz→클린빌드 main.js md5 동일 검증됨(`43a12b70e097dbee14dd00e40a440dd3`) |
...
```
# device_stage_files로 _dev_github/wordcraft_src_v1.4.20.tgz 스테이징
mkdir -p /home/claude/wcbuild && cd /home/claude/wcbuild
tar xzf "/mnt/user-data/uploads/.../wordcraft_src_v1.4.20.tgz"
...
2. 진실 확인: 라이브 version.json(캐시버스터=1.4.20) · RELEASE_LOG 맨 위(v1.4.20) · 로컬 `src_v1.4.20` APP_VERSION · **빌드 번들 grep**(L25).
3. 컨테이너 비었으면 §5로 소스 복구·빌드(tgz = v1.4.20).
